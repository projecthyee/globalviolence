...
cv_df_violence <-
  ...
  mutate(
    ...
    rmse_ridge = map2_dbl(
      ridge, test, \(mod, test) rmse_glmnet(mod, test, "violence_rate"))
  ) %>% 
...
cv_df_homicide <-
  ...
  mutate(
    ...
    lasso = map(train, \(df) fit_glmnet(df, outcome = "homicide_rate", 
                                        alpha = 1,lambda = lambda)),
    ridge = map(train, \(df) fit_glmnet(df, outcome = "homicide_rate", 
                                        alpha = 0, lambda = lambda))
  ) %>% 
  mutate(
    rmse_all_var = map2_dbl(
    ...
    rmse_ridge = map2_dbl(
      ridge, test, \(mod, test) rmse_glmnet(mod, test, "homicide_rate"))
  ) %>% 
  dplyr::select(starts_with("rmse_"))
```
