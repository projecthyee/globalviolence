...
best_subset = function(predictor, outcome, criterion) {
  ...
  optimal_subset <- 
    leaps(x = predictor, y = outcome, nbest = 3, 
      method = criterion, names = names(predictors))
  
  optimal_rsq <- optimal_subset$criterion %>% max()
  optimal_subset_idx <- optimal_subset$criterion %>% which.max()
  
  print(optimal_rsq) 
  print(optimal_subset$which[optimal_subset_idx,]) %>% knitr::kable()
...
subset_rsq_violence <- 
  leaps(x = violence_predictors, y = violence_outcome, nbest = 3, 
        method = "adjr2", names = names(violence_predictors)) 


subset_rsq_violence_max <- subset_rsq_violence$adjr2 %>% 
  max()
violence_selection_idx <- subset_rsq_violence$adjr2 %>% 
  which.max()

subset_rsq_violence_max
subset_rsq_violence$which[violence_selection_idx,] %>% knitr::kable()
```
