...
fit_glmnet = function(df, alpha, outcome, lambda) {
  predictor <- model.matrix(outcome ~., data = df, alpha = alpha)
...
homicide_matrix <- model.matrix(homicide_rate ~., data = homicide_df)
...
violence_matrix <- model.matrix(violence_rate ~., data = violence_df)
...
cv_df_violence <-
  ...
  mutate(
    ...
    lasso = map(train, \(df) fit_glmnet(df, alpha = 1)),
    ridge = map(train, \(df) fit_glmnet(df, alpha = 0))
  )
```
