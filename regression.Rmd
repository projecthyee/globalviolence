...
## check_model
```{r}
check_model = function(resid_list, observed, predicted, model_name) {
  diagnostics <- list()
  
  for (resid_df in resid_list) {
    qqplot <- 
      resid_df %>% 
        ...
        ylab("Residuals") +
        ggtitle(str_c("QQ-Plot for ", model_name))
    
    resid_fit <- 
      resid_df %>% 
        ggplot(aes(x = predicted, y = resid_df)) +
        geom_point() +
        geom_hline(yintercept = 0, linetype = "dotted") +
        xlab("Fitted Values") +
        ylab("Residuals") +
        ggtitle(str_c("Fitted Values vs Residuals for ", model_name))
