...
We perform this analysis to uncover the extent to which 
...
bimodal and slightly left-skewed. We will apply ln transformations and 
Box-Cox transformations to these variables, in order to account for concerns 
regarding the constancy of variance in linear models. Transformations are 
intened to approximately normalize the data.

## Transformations
Transformation step involves writing a function for natural log transformation 
and a function for Box-Cox transformation. The functions will be mapped into the 
...
the approximate normality of each of the predictor distribtuions. However, 
...
we ultimately decided that to use the original data. Concerns regarding the 
normality of the distributions are alleviated by the central limit theorem. 
With a suffeciently large amount of data points, we can assume that the sampling 
distribution is approximately. Using the convention that n > 30 allows for 
...
The optimal lambda for violence rate is 0.794. Based on lasso estimates, 
the coefficient for `gdp` was shrunk to 0. 
