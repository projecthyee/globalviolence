...
## Model Building 
Three types of models were made for the dataset: MLR, best subset, and lasso. 

(NOT SURE IF WE SHOULD PUT EVERYTHING. IF WE DO PUT EVERYTHING FROM REGRESSION 
FILE THIS IS GOING TO GET REALLY LONG)
...
### Comparing the RMSE for Violence 
...
predicting violence rate, we can see that they are relatively similar but MLR
including all predictor variables has a greater spread than the others with a 
higher upper bound in the violin plot. 

In general, each of the 3 models will not differ by many predictor variables due
to the few number of predictor variables in out dataset and thus perform similarly. 
Comparing the 3 models, the best subset regression model
...
### Comparing the RMSE for Homicide
...
Examining the 3 models we used to predict homicide rate, we can see that 
...
### Model Diagnostics for Best Subset 
