...
### Collinearity in Homicide Model
```{r }
...
#### Collinearity in Violence Model
```{r }
...
#### Predict Violence Rate using Cp 
```{r }
...
#### Collinearity Check in Best-Subset Violence Model 
```{r }
...
#### Predict Homicide Rate using Cp
```{r }
...
#### Collinearity Check in Best-Subset Homicide Model
```{r }
...
notice that our assumption of normality of residuals may be in violation, as 
there appear to many lower-bound and upper-bound outliers that do not fall along 
the reference line. Regarding the plots of fitted values versus residuals, we 
...
`r cat(violence_best_subset_rsq[["Criterion"]])` and included 
...
r-squared of 
`r cat(subset_homicide_lm %>% broom::glance() %>% pull(adj.r.squared))`. The 
