...
(I PARAPHRASED THE MOTIVATION IN OUT PROPOSAL DOCUMENT. MAYBE INCLUDE WHY WE CHOSE THIS TIME FRAME?)
...
### Average Homicide Rate Across Region
```{r}
homicide_visual_df |>
  filter(unit_of_measurement == "Rate per 100,000 population",) |>
...
homicide_visual_df |>
  ...
  summarize(num_homicide = sum(homicide_rate)) |>
  ...
  labs(x = "Category",
       ...
       Title = "Number of Homicides Across Category")
```
...
```{r}
# I took the average for each country and plotted using the choropleth. 
merged_violence_df |>
...
violence_visual_df |>
  ...
  ggplot(aes(y = avg_violence_rate, x = as.factor(year), fill = region)) +
  geom_col(position = "dodge", bin = 3.0) + 
  labs(x = "Year",
       y = "Average Homicide Rate",
       Title = "Average Homicide Rate Across Region")
...
violence_visual_df |>
  ...
  summarize(num_violence = sum(violence_rate)) |>
