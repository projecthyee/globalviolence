...
gdp_df = 
  ...
  dplyr::select(iso3_code = 2, "2015":"2023") |>
...
unemployment_df =
  ...
  dplyr::select(iso3_code = 4, "2013":"2023") |>
...
human_develop_df =
  ...
  dplyr::select(iso3_code = 1, "2015":"2022") |>
...
trafficking_df =
  ...
  dplyr::select(iso3_code : region, year, trafficked_victims = value) |>
...
alcohol_df =
  ...
  dplyr::select(iso3_code = spatial_dim_value_code, 
         year = period, 
...
homicide_visual_df |>
  ...
  summarize(avg_homicide_rate = mean(homicide_rate)) |>
  ggplot(aes(y = avg_homicide_rate, x = reorder(category, avg_homicide_rate),
             fill = sex)) +
  geom_col(position = "dodge", bin = 3.0) + 
...
merged_violence_df |> 
  ...
  dplyr::select(iso3_code, country, year, violence_rate) |> 
  group_by(year) |> 
  summarise(count = n()) |>
  knitr::kable()
```
We have the least NA values for the last 6 years or so (2017-2023), excluding 2019
and 2020, which may be due to COVID. Based on this, what should we dplyr::select?

#### NA Values by Country
```{r}
merged_violence_df |> 
  filter(is.na(violence_rate)) |> 
  dplyr::select(iso3_code, country, year, violence_rate) |> 
