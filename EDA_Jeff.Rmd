...
```{r}
make_plot = function(data, geom_type, title, ...) {
  plt <- data %>% 
    ggplot(aes(...)) +
    geom_type() + 
    ggtitle(title) +
...
for (var in var_interest) {
  make_plot(EDA_var_interest, geom_point, str_c("Homicide Rate by ", var), 
            x = !!sym(var), y = homicide_rate)
}
...
map(var_interest, \(x) make_plot(EDA_var_interest, geom_point, 
    str_c("Homicide Rate by ", x), x = (x), y = homicide_rate
    ))
