...
var_names <- EDA_var_interest %>% 
  select(c(human_development_index:alcohol_consumption)) %>% 
  colnames() 
...
```{r}
map(var_names, \(x) make_plot(EDA_var_interest, geom_point, 
    str_c("Average Violence Rate by ", x), x = .data[[x]], y = avg_violence_rate
    ))
...
map(var_names, \(x) make_plot(EDA_var_interest, geom_point,
    ...
    x = .data[[x]], y = avg_violence_rate, color = region)
    )
...
```{r}
map(var_names, \(x) make_plot(EDA_var_interest, geom_density, 
    str_c("Distribution of ", x, " based on Region"), "development_status",
...
EDA_var_interest %>% 
  ...
  make_plot(geom_boxplot, title = "Average Violence Rate by Development Status", 
            x = development_status, y = avg_violence_rate)
